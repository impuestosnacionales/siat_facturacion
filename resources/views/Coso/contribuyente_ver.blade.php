    ...
    <div class="row flex-nowrap">
        ...
        <div class="row">
            <div class="col col-md-12 col-sm-12 justify-content-center">
                ...
                <a href="{{route('contribuyente')}}" class="btn btn-primary" role="button">Volver a lista</a>
                <div class="card">
                @csrf
                    <input type="hidden" name="_token" value="{{csrf_token()}}"> 
                    <br>
  <div class="mb-3">
    <label class="form-label">Descripcion Producto Contribuyente: <strong>{{$contribuyente->Descripcion_Producto_Contribuyente}}</strong></label>
  </div>
  <div class="mb-3">
    <label class="form-label">Precio: <strong>{{$contribuyente->Precio}}</strong></label>
  </div>
  <div class="mb-3">
    <label class="form-label">Unidad de Medida: <strong>{{$contribuyente->Unidad_de_Medida}}</strong></label>
  </div>
  <input type="button" class="btn btn-primary" name="Submit" value="imprimir" onclick="javascript:window.print()">
  <br>
  <form action="{{route('contribuyente.destroy',$contribuyente->id)}}" method="POST">
    @csrf
    {{method_field('DELETE')}}
