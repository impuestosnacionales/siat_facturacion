...
<body>
    ...
    <div class="row flex-nowrap">
        ...
        <div class="row">
            <div class="col col-md-12 col-sm-12 justify-content-center">
                ...
                <a href="{{route('producto')}}" class="btn btn-primary" role="button">Volver a lista</a>
                <div class="card">
                @csrf
                    <input type="hidden" name="_token" value="{{csrf_token()}}"> 
                    <br>
  <div class="mb-3">
    <label class="form-label">Codigo Producto SIN:<strong>{{$producto->Codigo_Producto_SIN }}</strong></label>
  </div>
  <div class="mb-3">
    <label class="form-label">Codigo Actividad CAEB<strong>{{$producto->Codigo_Actividad_CAEB}}</strong></label>
  </div>
  <div class="mb-3">
    <label class="form-label">Descripcion o Producto SIN <strong>{{$producto->Descripcion_o_producto_SIN}}</strong></label>
  </div>
  <div class="mb-3">
    <label class="form-label">ID del contribuyente <strong>{{$producto->id_contribuyente}}</strong></label>
  </div>
  ...
  <form action="{{route('producto.destroy',$producto->id)}}" method="POST">
    @csrf
    {{method_field('DELETE')}}
    <input class="btn btn-danger form-control" type="submit" value="eliminar" onclick="return EliminarProducto('¿Desea eliminar este Producto?') ">
  </form>
